...
@Tag(name = "Lemmatization services",
    description = "Lemmatization services for Ukrainian language"
)
@RestController
@RequestMapping('')
@CompileStatic
class LemmatizeController extends BaseController {

    @Autowired
    LemmatizeService lemmatizeService

    @Operation(summary = "Lemmatize the text"
    )
    @ApiResponses(value = [
            @ApiResponse(responseCode = "200", description = "OK"),
            @ApiResponse(responseCode = "400", description = "Invalid request")
    ])
    @PostMapping(path="/lemmatize")
    def tokenize(
        @io.swagger.v3.oas.annotations.parameters.RequestBody(
            description='Body text; e.g<br>{"text": "Сьогодні у продажі. 12-те зібрання творів 1969 р. І. П. Котляревського."}', 
            required = true)
        @RequestBody LemmatizeRequest request) {
        
        validateRequest(request)

        try {
            List<List<String>> tokens = lemmatizeService.lemmatize(request.text)

            LemmatizeResponse response = new LemmatizeResponse(tokens: tokens)
    ...
    @CompileStatic
    static class LemmatizeResponse extends ResponseBase {
        @Schema(description="List of lemmas inside the list of sentences.")
        List<List<String>> tokens
    }

    @Schema(description="Request to lemmatize text.")
    @CompileStatic
    static class LemmatizeRequest extends RequestBase {
    }
}
